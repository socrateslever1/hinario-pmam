...
function QuickAccess() {
  ...
  return (
    <section className="bg-background px-4 py-5 text-foreground md:py-7">
      <div className="container px-0 md:px-4">
        <div className="mb-4">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1a3a2a]/65 dark:text-[#c4a84b]">QG Digital</p>
          <h2 className="mt-1 text-2xl font-black tracking-tight">Acesso rápido</h2>
        </div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {items.map((item) => (
            <Link key={item.label} href={item.href}>
              <Card className="h-full cursor-pointer border-border/50 bg-card py-0 shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#c4a84b]/50 hover:shadow-md">
...
function StudentNoticePanel() {
  ...
  return (
    <section className="bg-[#062417] px-4 py-5 text-[#f8f7f0] md:bg-background md:px-0 md:py-7 md:text-foreground">
      <div className="container">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {noticesQuery.data.map((notice: any) => (
            <Card key={notice.id} className="border-white/10 bg-[#0b3323]/80 py-0 text-white shadow-lg md:border-border/50 md:bg-card md:text-foreground">
              <CardContent className="p-4">
                ...
                <Button
                  size="sm"
                  className="mt-3 w-full bg-[#f0bd3a] text-xs font-black text-[#062417] hover:bg-[#d6b64c]"
...
function StudentHighlights() {
  ...
  return (
    <section className="bg-[#f5f2e8] px-4 py-5 md:px-0 md:py-7">
      <div className="container">
        <div className="mb-4">
          ...
          <h2 className="text-2xl font-black text-foreground md:text-3xl" style={{ fontFamily: "Merriweather, serif" }}>
            Alunos em Destaque
          </h2>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {highlights.map((item: any) => (
            <Card key={item.id} className="overflow-hidden border-border/50 bg-card py-0 shadow-sm">
              <CardContent className="p-4">
                <div className="mb-3 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold text-[#1a3a2a]">{item.nome_guerra}</p>
    ...
    <section className="bg-background py-6 md:py-8">
      <div className="container">
        <div className="mb-5 text-center md:mb-7">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-muted px-4 py-1.5">
            <Star className="h-4 w-4 text-[#c4a84b]" />
            <span className="text-xs font-semibold uppercase tracking-widest text-[#1a3a2a]">Identidade PMAM</span>
          </div>
            ...
            <Card key={item.title} className="h-full overflow-hidden border-border/50 bg-card py-0 shadow-sm hover:border-[#c4a84b]/50">
              ...
              <CardContent className="p-4 md:p-5">
                <div className="mb-3 flex items-center gap-2.5">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#1a3a2a]">
...
function HymnCollection({ hymns }: { hymns: any[] | undefined }) {
  ...
  return (
    <section className="bg-muted/25 py-6 md:py-8">
      <div className="container">
        <div className="mb-5 text-center">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#1a3a2a]/65">Módulo Hinário</p>
          <h2 className="mt-1 text-2xl font-bold text-foreground md:text-3xl" style={{ fontFamily: "Merriweather, serif" }}>
              ...
              <Link key={category.key} href={`/hinos?categoria=${category.key}`}>
                <Card className="h-full cursor-pointer border-border/50 py-0 transition-colors hover:border-[#c4a84b]/50">
                  <CardContent className="p-4 text-center">
                    <div className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-[#1a3a2a]/10">
                      <category.icon className="h-5 w-5 text-[#1a3a2a]" />
                    </div>
                    ...
                    <p className="mt-2 text-xs font-bold text-[#8a6900]">{count} item(ns)</p>
                  </CardContent>
                </Card>
              </Link>
            );
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            {featured.map((hymn: any) => (
              <Link key={hymn.id} href={`/hino/${hymn.id}`}>
                <Card className="h-full cursor-pointer overflow-hidden border-border/50 py-0 hover:border-[#c4a84b]/50">
                  <div className="h-1.5 bg-gradient-to-r from-[#1a3a2a] via-[#2d5a27] to-[#c4a84b]" />
                  <CardContent className="p-4">
                    <p className="text-xs font-bold uppercase tracking-wider text-[#8a6900]">Destaque do acervo</p>
        ...
        <div className="mt-5 text-center">
          <Link href="/hinos">
            <Button className="bg-[#1a3a2a] text-white hover:bg-[#1a3a2a]/90">
