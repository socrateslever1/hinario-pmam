...
export default function HymnDetail({ catalog = "hymns" }: HymnDetailProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {/* Header */}
      <section className="military-gradient py-3 md:py-4">
        <div className="container">
          <Link href={catalogHref}>
            <Button
              variant="ghost"
              size="sm"
              className="mb-2 h-auto gap-2 px-0 py-1 text-xs text-white/70 hover:bg-transparent hover:text-white"
            >
          ...
          <div className="flex items-center gap-3">
            <div
              className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-lg text-sm font-bold text-white md:h-12 md:w-12 md:text-base"
              style={{ backgroundColor: catColor }}
            >
              {String(hymn.number).padStart(2, "0")}
            </div>
            <div className="min-w-0">
              <p className="mb-0.5 text-[10px] font-bold uppercase tracking-wider text-[#c4a84b] md:text-xs">
                {categoryLabels[hymn.category] || hymn.category}
              </p>
              <h1
                className="truncate text-xl font-bold leading-tight text-white md:text-2xl"
        ...
        <div className="checkerboard-pattern mt-3 w-full" />
      </section>
