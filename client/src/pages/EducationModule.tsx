...
function scoreLabel(score: number | null) {
  ...
  if (score >= 80) return "Aprovacao forte";
  if (score >= 70) return "Aprovado";
  if (score >= 50) return "Em recuperacao";
...
export default function EducationModule({ params }: EducationModuleProps) {
  ...
  const studyUnits = useMemo(() => (module && fullText ? extractStudyUnits(module, fullText) : []), [module, fullText]);
  const questionBank = useMemo(() => (module ? buildQuestionBank(module, studyUnits) : []), [module, studyUnits]);
  ...
  const filteredStudyUnits = studyUnits.filter((unit) => {
    const normalizedQuery = studyQuery.trim().toLowerCase();
  ...
  if (!studentNumber || !progress || !compiledModule) {
    return (
        ...
        <section className="py-16">
          <div className="container max-w-2xl">
            <Card className="border-[#c4a84b]/40 bg-[#c4a84b]/5">
              <CardHeader>
                <CardTitle style={{ fontFamily: "Merriweather, serif" }}>Identificacao obrigatoria</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 text-sm text-muted-foreground">
                <p>
                  Para abrir o modulo e salvar o progresso pessoal, primeiro informe o numero do aluno em
  ...
  const answeredCount = getAnsweredCount(compiledModule, progress.answers);
  const snippets = buildStudySnippets(fullText, consultQuery);
  ...
  const submitQuiz = () => {
    ...
    toast.success(`Avaliacao concluida: ${result.percentage}% (${result.correct}/${result.total}).`);
  };
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant="outline" className="border-white/20 text-white/80">{module.pages} paginas</Badge>
            ...
            <Card className="border-white/10 bg-white/10 text-white backdrop-blur">
              <CardContent className="space-y-3 p-5">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="rounded-xl border border-white/10 px-3 py-2">
                    <p className="text-xs uppercase tracking-[0.18em] text-white/50">Ultima nota</p>
        ...
        <div className="container space-y-6">
          <div className="grid gap-4 md:grid-cols-3">
            <Card>
              <CardContent className="flex items-center gap-4 p-5">
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Leitura concluida</p>
            ...
            <Card>
              <CardContent className="flex items-center gap-4 p-5">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{answeredCount}/{questionBank.length}</p>
                </div>
              </CardContent>
            </Card>
            <Card>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="h-auto w-full flex-wrap justify-start gap-2 rounded-2xl bg-muted/40 p-2">
              ...
              <TabsTrigger value="avaliacao" className="min-w-[150px]">Avalia??o</TabsTrigger>
            </TabsList>
              ...
              <div className="space-y-5">
                <Card className="border-[#c4a84b]/40 bg-[#c4a84b]/5">
                  <CardContent className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_repeat(3,auto)] lg:items-center">
                    ...
                    <div className="rounded-xl border bg-white px-4 py-2 text-sm text-muted-foreground"><strong className="text-foreground">{questionBank.length}</strong> quest?es</div>
                    <div className="rounded-xl border bg-white px-4 py-2 text-sm text-muted-foreground">P?gina <strong className="text-foreground">{safeStudyPage}</strong> de <strong className="text-foreground">{studyPageCount}</strong></div>
                  </CardContent>
                </Card>
                  ...
                  <div className="space-y-4">
                            ...
                            <AccordionContent className="px-5 pb-5">
                              <div className="space-y-4">
                                <div className="rounded-2xl border border-[#1a3a2a]/15 bg-[#1a3a2a]/5 p-4">
                                  <p className="text-xs uppercase tracking-[0.2em] text-[#1a3a2a]">Explica??o simples</p>
                    ...
                    {studyPageCount > 1 && (
                      <div className="flex flex-col gap-3 rounded-2xl border bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-sm text-muted-foreground">Navega??o do estudo completo</p>
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="outline" disabled={safeStudyPage >= studyPageCount} onClick={() => setStudyPage((current) => Math.min(studyPageCount, current + 1))}>Pr?xima<ChevronRight className="ml-2 h-4 w-4" /></Button>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4 xl:sticky xl:top-24 xl:self-start">
                    <Card>
                      ...
                      <CardContent className="space-y-3 text-sm text-muted-foreground">
                        <p>Este m?dulo foi aberto a partir do pr?prio PDF que voc? enviou.</p>
                        <p>Nos regulamentos, a leitura fica artigo por artigo. No manual, a navega??o fica por t?picos principais.</p>
                        <p>Cada bloco traz a base do documento, uma explica??o simples e palavras-chave para memoriza??o.</p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader><CardTitle style={{ fontFamily: "Merriweather, serif" }}>Objetivos do m?dulo</CardTitle></CardHeader>
                    ...
                    <Card>
                      <CardHeader><CardTitle style={{ fontFamily: "Merriweather, serif" }}>Pontos de mem?ria</CardTitle></CardHeader>
                ...
                {quizPageCount > 1 && (
                  <div className="flex flex-col gap-3 rounded-2xl border bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-sm text-muted-foreground">Navega??o do banco de quest?es</p>
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="outline" disabled={safeQuizPage >= quizPageCount} onClick={() => setQuizPage((current) => Math.min(quizPageCount, current + 1))}>Pr?xima<ChevronRight className="ml-2 h-4 w-4" /></Button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <TabsContent value="consulta">
              <div className="space-y-4">
                  ...
                  <CardContent className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center">
                    <div className="relative">
                      ...
                      <Input
                        ...
                        placeholder="Buscar no texto limpo extra?do"
                        className="pl-9"
                      />
                    </div>
                ...
                {consultQuery.trim() && snippets.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle style={{ fontFamily: "Merriweather, serif" }}>Ocorrencias encontradas</CardTitle>
                    </CardHeader>
                ...
                <div className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
                  <Card className="overflow-hidden border-border/60">
                    <CardHeader>
                      ...
                      <p className="text-sm text-muted-foreground">Visualizacao fiel do documento, preservando ilustracoes, tabelas e diagramação.</p>
                    </CardHeader>
                    <CardContent>
                      {pdfPath ? (
                        <iframe
                        ...
                        <div className="rounded-2xl border bg-muted/20 p-4 text-sm text-muted-foreground">PDF original indisponivel neste modulo.</div>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card className="border-border/60">
                    <CardHeader>
                      ...
                      <p className="text-sm text-muted-foreground">Vers?o otimizada para leitura, com os par?grafos reorganizados para consulta r?pida.</p>
                    </CardHeader>
                    <CardContent>
                      <div className="max-h-[72vh] min-h-[420px] overflow-auto rounded-2xl border bg-slate-50 p-4">
                        {isLoadingText ? (
                          <p className="text-sm text-muted-foreground">Carregando texto do m?dulo...</p>
                        ) : fullText ? (
                          ...
                          <p className="text-sm text-muted-foreground">N?o foi poss?vel carregar o texto extra?do deste material.</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
              ...
              <div className="space-y-5">
                <Card className="border-[#c4a84b]/40 bg-[#c4a84b]/5">
                  <CardContent className="grid gap-4 p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
                    <div>
                      <p className="text-sm text-muted-foreground">Voc? respondeu {answeredCount} de {questionBank.length} perguntas.</p>
                      <Progress value={Math.round((answeredCount / questionBank.length) * 100)} className="mt-3 [&>*]:bg-[#1a3a2a]" />
                      <p className="mt-3 text-sm text-muted-foreground">Ultima nota: <strong className="text-foreground">{currentScore ?? 0}%</strong> | Melhor nota: <strong className="text-foreground">{bestScore ?? 0}%</strong></p>
                    </div>
                    ...
                    return (
                      <Card key={question.id} className="border-border/60">
                        <CardHeader className="space-y-2">
                          ...
                          <p className="text-sm text-muted-foreground">Referencia: {question.reference}</p>
                        </CardHeader>
                        <CardContent className="space-y-3">
