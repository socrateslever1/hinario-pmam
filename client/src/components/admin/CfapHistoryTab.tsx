...
import { BookOpen, ExternalLink, Save, Search, Video } from "lucide-react";
...
export function CfapHistoryTab() {
  ...
  useEffect(() => {
    if (selected) setEditor(commanderToEditor(selected));
  }, [selected?.slug, query.data]);

  const save = trpc.cfapHistory.upsert.useMutation({
    onSuccess: async () => {
      toast.success("Registro histórico salvo.");
  ...
  if (!editor || !selected) return <p className="py-10 text-center text-sm text-muted-foreground">Carregando acervo histórico...</p>;

  return (
    <div className="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
      <Card className="h-fit border-border/60">
        <CardContent className="p-4">
          <div className="mb-3 flex items-center gap-2"><BookOpen className="h-5 w-5 text-[#9f8123]" /><h2 className="font-bold">Homenageados</h2></div>
          ...
          <div className="max-h-[62vh] space-y-1 overflow-y-auto pr-1">
            {filtered.map((commander) => (
              <button key={commander.slug} type="button" onClick={() => setSelectedSlug(commander.slug)} className={`w-full rounded-lg border px-3 py-2 text-left transition ${commander.slug === selected.slug ? "border-[#c4a84b] bg-[#c4a84b]/10" : "border-transparent hover:bg-muted"}`}>
      ...
      <Card className="border-border/60">
        <CardContent className="space-y-5 p-4 md:p-6">
          <div className="grid gap-5 md:grid-cols-[190px_1fr]">
            <CommanderPortrait name={editor.name} portraitIndex={selected.portraitIndex} portraitUrl={editor.portraitUrl} className="rounded-2xl border" />
          ...
          <div className="flex justify-end"><Button onClick={handleSave} disabled={save.isPending} className="w-full gap-2 bg-[#1a3a2a] text-white sm:w-auto"><Save className="h-4 w-4" />{save.isPending ? "Salvando..." : "Salvar homenageado"}</Button></div>
        </CardContent>
      </Card>
    </div>
  );
