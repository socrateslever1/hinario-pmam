      ...
      <div className="container py-3 md:py-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[1.2fr_1fr_1fr] md:items-start md:gap-8">
          <div className="flex items-start gap-2.5">
            <img src={LOGO_URL} alt="Brasão PMAM" className="h-8 w-8 object-contain md:h-9 md:w-9" />
            <div>
              ...
              <p className="mt-0.5 max-w-sm text-[11px] leading-snug text-white/60 md:text-xs">
                {settings?.footer_text || "Setores digitais para gestão, formação, comunicação e rotina militar"}
              </p>
          ...
          <div className="hidden sm:block">
            ...
            <ul className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px] md:block md:space-y-1 md:text-xs">
              ...
              <li><Link href="/sala-de-aula" className="text-white/60 transition-colors no-underline hover:text-[#c4a84b]">Sala de Aula</Link></li>
              <li><Link href="/hinos" className="text-white/60 transition-colors no-underline hover:text-[#c4a84b]">Hinos e Canções</Link></li>
              ...
              <li><Link href="/sobre" className="text-white/60 transition-colors no-underline hover:text-[#c4a84b]">Portfólio do QG Digital</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-1 text-[11px] sm:block sm:space-y-1 md:text-xs">
              {settings?.footer_phone && <p className="flex items-center gap-2 text-white/60"><Phone className="h-3 w-3 shrink-0 text-[#c4a84b]" />{settings.footer_phone}</p>}
              {settings?.footer_email && <p className="flex items-center gap-2 text-white/60"><Mail className="h-3 w-3 shrink-0 text-[#c4a84b]" />{settings.footer_email}</p>}
              {settings?.footer_address && <p className="flex items-center gap-2 text-white/60"><MapPin className="h-3 w-3 shrink-0 text-[#c4a84b]" />{settings.footer_address}</p>}
              {(settings?.footer_instagram || settings?.footer_facebook) && (
                <div className="mt-1 flex items-center gap-3">
                  {settings?.footer_instagram && <a href={settings.footer_instagram} target="_blank" rel="noopener noreferrer" className="text-white/60 transition-colors hover:text-[#c4a84b]"><Instagram className="h-4 w-4" /></a>}
                  {settings?.footer_facebook && <a href={settings.footer_facebook} target="_blank" rel="noopener noreferrer" className="text-white/60 transition-colors hover:text-[#c4a84b]"><Facebook className="h-4 w-4" /></a>}
                </div>
              )}
        ...
        <div className="mt-3 border-t border-white/10 pt-2 text-center text-[10px] leading-tight text-white/40 md:mt-4 md:text-[11px]">
          ...
          <p className="mt-0.5 flex items-center justify-center gap-1"><Shield className="h-3 w-3" /> Servir e Proteger</p>
        </div>
      </div>
    </footer>
  );
